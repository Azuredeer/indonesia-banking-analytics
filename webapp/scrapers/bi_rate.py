...
# Nama bulan lengkap Bahasa Indonesia untuk keperluan tampilan (kebalikan
# dari _BULAN_MAP di atas, dibatasi ke nama lengkap saja, bukan singkatan).
_BULAN_NAMA_LENGKAP = {
...
def format_tanggal_indonesia(ts) -> str:
    """
    Format pd.Timestamp jadi label 'D Bulan YYYY' berbahasa Indonesia, tidak
    bergantung locale sistem (strftime('%B') bisa keluar nama bulan Inggris
    kalau locale server tidak di-set ke id_ID).
    """
    ts = pd.Timestamp(ts)
    return f"{ts.day} {_BULAN_NAMA_LENGKAP[ts.month]} {ts.year}"
