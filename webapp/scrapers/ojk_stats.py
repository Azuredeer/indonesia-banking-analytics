...
# Dipakai untuk mengenali periode (bulan-tahun) dari judul laporan, mis.
# "Statistik Perbankan Indonesia - Juli 2025" -> Timestamp(2025-07-01).
# Ini membuat hasil pencarian bisa diurutkan kronologis & memudahkan
# analisis time-series tanpa perlu buka satu-satu setiap file.
_BULAN_MAP = {
    "januari": 1, "februari": 2, "maret": 3, "april": 4, "mei": 5, "juni": 6,
    "juli": 7, "agustus": 8, "september": 9, "oktober": 10, "november": 11, "desember": 12,
}
_BULAN_NAMA = {v: k.capitalize() for k, v in _BULAN_MAP.items()}
_PERIODE_PATTERN = re.compile(
    r"(" + "|".join(_BULAN_MAP.keys()) + r")\s+(\d{4})", re.IGNORECASE
)


def _format_periode_id(periode: pd.Timestamp) -> str:
    """Format Timestamp jadi label 'Bulan YYYY' berbahasa Indonesia, tidak
    bergantung locale sistem (strftime('%B') bisa keluar Inggris di server)."""
    return f"{_BULAN_NAMA[periode.month]} {periode.year}"


def _extract_periode(judul: str):
    """Coba tebak periode (bulan-tahun) dari judul laporan. Return pd.Timestamp
    di tanggal 1 bulan tsb, atau None kalau tidak ketemu pola bulan-tahun."""
    if not judul:
        return None
    m = _PERIODE_PATTERN.search(judul)
    if not m:
        return None
    bulan_name, tahun = m.groups()
    bulan = _BULAN_MAP.get(bulan_name.lower())
    if not bulan:
        return None
    try:
        return pd.Timestamp(year=int(tahun), month=bulan, day=1)
    except ValueError:
        return None
...
def search_reports_multi(keyword: str | None = None, category_keys: list[str] | None = None) -> list[dict]:
    ...
    for key in category_keys:
        ...
        for r in reports:
            ...
            periode = _extract_periode(r["judul"])
            r_tagged["periode"] = _format_periode_id(periode) if periode is not None else "-"
            r_tagged["_periode_sort"] = periode if periode is not None else pd.Timestamp.min
            all_results.append(r_tagged)

    # Urutkan dari periode terbaru ke terlama supaya gampang dianalisis
    # secara kronologis; laporan tanpa periode terdeteksi ditaruh di akhir.
    all_results.sort(key=lambda r: r["_periode_sort"], reverse=True)
